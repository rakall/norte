...
resource "azurerm_public_ip_prefix" "ips_prefix" {
  for_each = local.prefix_name_list
...
module "pips" {
  ...
  ip_prefix_assignment = var.ip_prefix_assignment
  channel              = var.channel
  cia                  = var.cia
  description          = var.description
  tracking_code        = var.tracking_code
  module_depends_on    = [azurerm_public_ip_prefix.ips_prefix]
}
data "azurerm_public_ip" "module-pips" {
  count               = length(module.pips.pip_names)
  name                = module.pips.pip_names[count.index]
  resource_group_name = var.resource_group
...
locals {
  prefix_name_list    = toset(compact(var.ip_prefix_assignment[*].prefix_name))
...
resource "azurerm_linux_virtual_machine_scale_set" "linux_nvavss" {
  ...
  depends_on = [data.azurerm_public_ip_prefix.data_pip_prefix, azurerm_lb_backend_address_pool.ilb_pool, azurerm_lb_backend_address_pool.elb_pool, azurerm_lb_rule.elb_rule]
}
