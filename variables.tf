locals {
  ...
  product                    = "HubNVA"
  cia                        = "AHC"
  nva_name                   = "${var.entity}${var.environment}${lookup(var.location_short, var.location)}nva${lookup(var.product_name_short, var.product_name)}gen${local.product_purpose}${lookup(var.nvaId, var.product_name)}"
  ...
  configurationTemplate      = var.nic_template_name
}
...
variable "nic_template_name" {
  type        = string
  description = "(Required) Configiration template name"
}
